...
6. **Run post-implementation checks**: `scripts/agent-helpers/post-implementation-checks [git-ref]`. Review the report (Auto-Fixed, Lint Issues, Test Failures), fix every reported issue, re-run the script. Repeat until all sections show "None". Then run `flutter build` to verify.
