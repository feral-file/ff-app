...
void main() {
  group('PlaylistsProvider (family by PlaylistType)', () {
    ...
    test('addressBased provider returns initial state with empty playlists', () {
      final db = AppDatabase.forTesting(NativeDatabase.memory());
      addTearDown(db.close);
      final dbService = DatabaseService(db);
      ...
      final state =
          container.read(playlistsProvider(PlaylistType.addressBased));
    ...
    test('loadPlaylists for addressBased completes and updates state', () async {
      final db = AppDatabase.forTesting(NativeDatabase.memory());
      addTearDown(db.close);
      final dbService = DatabaseService(db);
