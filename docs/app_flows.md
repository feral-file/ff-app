...
- notes — **FF1 setup orchestration (`ff1SetupOrchestratorProvider`):**
  ...
  - **`completeSession(FF1Device)`** (when `activeSession != null`): completes onboarding, then calls `tearDownAfterSetupComplete()` so the shared cleanup path hides the pairing QR when possible, disconnects BLE best-effort, resets connect/Wi‑Fi notifiers and orchestrator ephemeral state, then **`GoRouter.go`** to device configuration (unless the caller passes `shouldNavigate: false`). **Connect FF1** passes `shouldNavigate: !connected.portalIsSet` (orchestrator navigates when portal is not set; portal-all-set defers navigation until the user confirms on-screen). Returns `false` if there is no active session or completion is already in progress; otherwise returns `true`. Serialized with `_sessionCompletionInProgress`; failures rethrow so the UI can recover.
...
- notes — **Internet-ready persistence (no guided session):** `ConnectFF1Notifier` persists the FF1 and promotes it active in `_handlePostConnect` when the device is already internet-ready, so “setup complete” does not race an empty device store. Guided `completeSession` now only runs **completeOnboarding** plus the shared teardown because persistence already happened before the success effect reached the UI.
