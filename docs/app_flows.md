...
- notes — **FF1 setup orchestration (`ff1SetupOrchestratorProvider`):**
  ...
  - **`completeSession(FF1Device)`** (when `activeSession != null`): persists the device, calls `completeOnboarding()`, hides pairing QR over relayer when `topicId` is set, disconnects BLE best-effort, resets connect/Wi‑Fi notifiers and orchestrator ephemeral state, then **`GoRouter.go`** to device configuration (unless the caller passes `shouldNavigate: false`). **Connect FF1** passes `shouldNavigate: !connected.portalIsSet` (orchestrator navigates when portal is not set; portal-all-set defers navigation until the user confirms on-screen). Returns `false` if there is no active session or completion is already in progress; otherwise returns `true`. Serialized with `_sessionCompletionInProgress`; failures rethrow so the UI can recover.
