...
- failure/edge states:
  ...
  - loading overlay only when playlist id or item list from FF1 changes; index/pause/sleep-only updates reuse cached rows without flashing loading or re-running SQLite/indexer for the same list
