...
- notes — **FF1 setup orchestration (`ff1SetupOrchestratorProvider`):**
  ...
  - **`completeSession(FF1Device)`** (when `activeSession != null`): persists the device, calls `completeOnboarding()`, hides pairing QR over relayer when `topicId` is set, disconnects BLE best-effort, resets connect/Wi‑Fi notifiers and orchestrator ephemeral state, then **`GoRouter.go`** to device configuration (unless the caller passes `shouldNavigate: false` — used on **Connect FF1** so the screen controls navigation, including portal-all-set). Returns `false` if there is no active session or completion is already in progress; otherwise returns `true`. Serialized with `_sessionCompletionInProgress`; failures rethrow so the UI can recover.
...
- notes / caveats:
  ...
  - **`FF1SetupInternetReady` with `activeSession`:** `ConnectFF1Page` calls `completeSession(connected.ff1device, shouldNavigate: false)` so the orchestrator does not navigate internally; the screen performs `GoRouter.replace` to device configuration when appropriate. If **portal is set**, the user sees the portal “all set” state and **Go to Settings** runs `replace` after guided completion (button stays disabled while completion is in progress).
