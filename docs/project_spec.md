...
- Pairing QR (Show/Hide) on Device Configuration: the control label tracks relayer `device_status` when `displayURL` reflects the Chromium pairing step; FF1 builds that omit `displayURL` are treated as pairing-QR visible for that label. The setup orchestration may hide the pairing-QR action before this screen during the initial setup visit; that path does not depend on changing the default local toggle value (see `docs/app_flows.md`, DeviceConfigScreen). In-flight show/hide commands apply only to the FF1 that was active when the command started, so switching the active device does not adopt completion from the previous selection.
